...
public class TransactionController {
    ...
    public ResponseEntity<Map<String, Object>> getTransactions(
            ...
            @Parameter(description = "Page number (zero-based). Default: 0")
            @RequestParam(defaultValue = "0") int page,

            @Parameter(description = "Page size. Default: 10, Max: 100")
            @RequestParam(defaultValue = "10") int size,

            @Parameter(description = "Filter by type: SEND | REQUEST | ADD_MONEY | WITHDRAW | LOAN_REPAYMENT")
            @RequestParam(required = false) TransactionType type,

            @Parameter(description = "Filter by status: PENDING | SUCCESS | FAILED | CANCELLED")
            ...
            @Parameter(description = "Search by counterparty name, email, or transaction ID")
            @RequestParam(required = false) String search

    ) {
        Map<String, Object> response =
                transactionService.getTransactions(page, size, type, status, from, to, search);
    ...
    @Operation(
            ...
            description = "Fetch full details of a single transaction by its ID. " +
    ...
    @Operation(
            ...
            description = "Returns the last 5 transactions for the authenticated user. " +
